#!/bin/sh
#
#     This script will setup Salix booting from disk (USB or harddrive)
#
...
# if we're running this from X, re-run the script in sakura or xterm
if [ "$DISPLAY" != "" ]; then
...
if [ "$(fdisk -l "$DEV" | fgrep "$DEV" | fgrep "*")" != "" ]; then
   ...
   echo "will boot only Salix Linux by default."
